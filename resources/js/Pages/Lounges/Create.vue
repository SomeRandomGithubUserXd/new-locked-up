...
const lounge = useForm({
    ...
    name_en: '',
    is_active: true,
    lounge_schedule_id: null,
})

const update = () => {
    lounge.post(route('lounges.store'), {
        onError: (errs) => {
            console.log(errs)
        }
    })
...
<template>
    <Head title="Добавить лаунж"/>

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Добавить лаунж</h2>
        </template>
