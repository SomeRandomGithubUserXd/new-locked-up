...
<template>
    <form class="space-y-6" @submit.prevent="emit('submit')">
        ...
        <div class="grid grid-cols-6 gap-6">
            ...
            <div class="col-span-6 sm:col-span-6">
                <InputLabel for="time" :value="`Время (${locale})`"/>
                <TextInput
                    id="time"
                    type="text"
                    class="mt-1 block w-full"
                    v-model="modelValue[getAttribute('time')]"
                />
                <InputError class="mt-2" :message="modelValue.errors[getAttribute('time')]"/>
            </div>
            <div class="col-span-6 sm:col-span-6">
            ...
            <div class="col-span-6 sm:col-span-6">
                <InputLabel for="full_description" :value="`Подробное описание (${locale})`"/>
                <textarea
                    id="full_description"
                    v-model="modelValue[getAttribute('full_description')]"
                    class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </textarea>
                <InputError class="mt-2" :message="modelValue.errors[getAttribute('full_description')]"/>
            </div>
            <div class="col-span-6 sm:col-span-6">
