...
class OrderResource extends JsonResource
{
    ...
    public static function singleItem(Order $order): array
    {
        return [
            ...
            'source' => $order->order_source_id,
            'promo_code' => Sale::firstWhere(['promo_code' => $order->promo]),
            'certificate' => Certificate::find($order->certificate_id),
            'comment' => $order->comment,
            'fact_payment' => $order->postpaid ?? 0,
            'pre_payed' => $order->pre_paid ?? 0,
            'online_payment' => $order->paid_through_acquiring ?? 0,
            'payed_aggregator' => $order->paid_through_aggregator ?? 0,
            'status' => $order->status ?? 0,
            'price_total' => $order->price_total,
            'option' => $order->package ?? Order::$packageOptions[0],
            'fact_payment_type' => $order->postpaid_type ?? '',
            'pre_payed_type' => $order->pre_paid_type ?? '',
